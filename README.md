...
This code is not a direct representation of Attest's code or coding principles; it is designed to guide the technical stage interview. You are asked to familiarise yourself with the existing code - you can make notes on improvements you may make given you had the opportunity to do so. Do not make changes to the code prior to the interview - we will be making changes together during the interview.

You will be asked to do some technical tasks during the interview to add some new functionality of the mini-application. Prior to the interview please ensure that you have familarised yourself with the application, code and concepts.
...
The sidebar on the right displays and applies demographic filters, this is used to filter the response data, i.e. clicking "London" will change the responses shown on the left to only display the responses of respondents from London.
...
During the interview you will be required to mutate the application state and extend the filtering functionality; so it is recommended that you have a strong grasp of any existing state management patterns and respondent filtering used within the code base as well as the data models/types involved.
